...
- GitHub SLSA provenance naming the exact checked-out commit, including when the workflow's own default-branch revision is newer.
...
``` bash
gh attestation verify example-1.2.3.gem \
  --repo OWNER/REPOSITORY \
  --source-digest MERGED_SHA \
  --signer-workflow OWNER/REPOSITORY/.github/workflows/release-publish.yaml
...
Native GitHub records the merge and any bypass; the artifact receipt includes the PR and merging actor. This version does not export organization audit-log evidence or infer bypass reasons from review counts.
